...
Sequelize, or SQL requires the database or table to be present before a CRUD op can be done. This is not the case with MongoDB. A create call in MongoDB call create all preceding layers automatically, if they don't exist.


## Creating the models
Since we don't have a SQL database now, and so, the Sequelize models will not work. Let's create the Product model again (we'll use a class again, just like we did earlier while using files for storage):
```js
class Product {
